    ...
    <div class="estate-main" :class="{ 'full-list': !selectedEstate }">
      ...
      <div class="estate-details" v-if="selectedEstate">
        ...
        <div class="estate-details-actions">
          <button class="photo-btn">{{ $t('estate.photo') }}</button>
          <button class="plan-btn">{{ $t('estate.plan') }}</button>
          <button 
            v-if="isLoggedIn"
            class="subscribe-btn" 
          ...
          >
            {{ subscribing ? $t('estate.subscribing') : (isSubscribed ? $t('estate.subscribed') : $t('estate.subscribe')) }}
          </button>
          <button 
            v-else
            ...
            @click="goToLogin"
          >
            {{ $t('estate.loginToSubscribe') }}
          </button>
        </div>
        <div class="estate-details-description">
          <strong>{{ $t('estate.description') }}</strong>
...
// API-аас үл хөдлөх хөрөнгийн өгөгдөл авах
const fetchEstates = async () => {
  try {
    console.log('Fetching estates from API...'); // Debug log
    const response = await fetch('http://localhost:3001/api/estates');
    const data = await response.json();
    console.log('API response:', data); // Debug log
    
    if (data.success) {
      estates.value = data.estates.map(estate => ({
        id: estate._id,
        title: estate.title,
        ...
        petsAllowed: estate.petsAllowed,
        allowed: estate.petsAllowed, // Legacy field for template compatibility
        price: estate.price,
        fav: estate.fav || false,
        image: estate.image,
        description: estate.description,
        propertyType: estate.propertyType
      }));
      console.log('Mapped estates:', estates.value); // Debug log
    } else {
      console.error('API returned error:', data.error);
    }
  } catch (error) {
    console.error('Error fetching estates:', error);
    // Fallback: хуучин mock data
    estates.value = [
      {
        id: 1,
...
.estate-page {
  ...
  min-height: 100vh;
  border-radius: 24px;
  overflow: visible; /* Filter dropdown-д зориулсан */
}
.estate-filters {
  display: flex;
  gap: 0.7rem;
  ...
  position: relative;
  z-index: 10; /* Dropdown-ууд харагдахын тулд */
  overflow: visible;
}
...
.estate-filters input {
  ...
  background: #fff;
  position: relative;
  z-index: 15; /* Select dropdown-ууд дээд талд гарахын тулд */
  min-width: 100px; /* Minimum ширээ өгөх */
}
.save-search {
  background: #ffe066;
  border: none;
